...
The ratio quantities are useful for comparisons to other \pbpb\ measurements; $\Delta_{\Theta(\rvar)}$ is comparable to $\DeltaDptr$, however it is integrated over charged-particle \pt\ in the \mbox{1--4 GeV} interval \cite{Aaboud:2018hpb}.
...
The flattening of the \RP\ distributions at large distances suggests that while wider jets have a softer fragmentation and contain more particles with less \pt\ in \pbpb\ compared to \pp\ \mbox{collisions \cite{Chesler2016, Hulcher:2017cpt}}, this effect plateaus for jets with radius larger than 0.6.
